...
%
The \texttt{Sweep Metric Algorithm}, presented in the next section, 
will try to go behind this limits.
%
...
%
All the remained images will be assigned a score.
...
the robot current orientation will be assigned a higher value.
%
...
%
The approach followed here is the same of the \textit{spacial metric 
algorithm} presented in the previous section: 
%
The preferable image would be distant a predefined quantity
from the robot, specified by the user. This value
indicate us where to centre another Gaussian function, in 
order to obtain the score from the distance between image and 
robot. The more the image is close to the preferred distance, 
the more the score assigned will be high.
%
