...
from \texttt{Robot} class. They have to implement, at least,
the \texttt{DrawRobot()} father's pure virtual method,
...
This class allows to draw a \morduc{} model,
in order to implement the exocentric
vision with this specific robot (see section 
\ref{intro:3morduc}). Regardless of how data and images
...
\\
\begin{lstlisting}[caption={\texttt{Morduc::DrawRobot()} method}, label={code:drawrobot}]
...
\\
As you have surely observed, \texttt{Morduc} contains also 
two private methods - 
...
a disk and a cylinder.
\texttt{radius}, instead, stores the radius value 
for the disks making up the robot.
After some calibration, its default value 
