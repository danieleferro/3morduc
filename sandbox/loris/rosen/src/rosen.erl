...
-define (GRAPHICAL_ENGINE, graphical_engine).
...
%%
add_object (Object) ->
%%     Result = rpc:call (right_display@skip,
%% 		       graphical_engine, 
%% 		       add_object,
%% 		       [right_display_fsm,Object]),

%%     case #engine_state.stereo of
%% 	true ->
%% 	    graphical_engine:add_object(right_display,
%% 					Object),
%% 	    graphical_engine:add_object(left_display,
%% 					Object);
%% 	_ ->
%% 	    graphical_engine:add_object(display,
%% 					Object)
%%     end.
...
%%
  idle_draw ({add_object, Pid}, StateData) ->
    ...
    DL = StateData#engine_state.left_display,
    DR = StateData#engine_state.right_display,
    
%% %%     EventResultLeft = rpc:call(DL, graphical_engine
    io:format("~p~n",[DL]),
    EventResultLeft = gen_fsm:send_event (DL, {add_object, Pid}),
    EventResultRight = gen_fsm:send_event (DR, {add_object, Pid}),
    
    io:format("~p~n~p~n",[EventResultLeft, EventResultRight]),
    io:format("add_object event sent!~n"),
...
%% Update Time Parameters
    ...
    {next_state, idle_draw, NewStateData, NewStateTime#engine_state.period}.
...
%%
init (Config) ->    
    io:format("Initializing rosen main engine... ~n"),
    
    CollisionOptions = proplists:get_value (collision, Config),
    TimeType = proplists:get_value (timeType, Config),
    
    case lists:member(stereo, Config) of
	true ->
	    Stereo = true,
	    {ok, LeftNode} = slave:start (net_adm:localhost(), 
					  'left_display',
					  " --env DISPLAY=:0 " ++
					  " -pa ebin/ -pa include/ -pa examples/ "),
	    
	    {ok, DL} = rpc:call (LeftNode,
				 graphical_engine,
				 start_link, 
				 [[{global, left_display_fsm}, 
				   {name, left_display}]]),

	    io:format (" ~p initialized~n", [DL]),
	    
 	    {ok, RightNode} = slave:start (net_adm:localhost(), 
				    'right_display',
				    " --env DISPLAY-:0 " ++
				    " -pa ebin/ -pa include/ -pa examples/ "),

	    {ok, DR} = rpc:call (RightNode,
				 graphical_engine, 
				 start_link, 
				 [[{global, right_display_fsm},{name, right_display}]]),

	    io:format ("Left  Node: ~p~n", [DL]),
	    io:format ("Right Node: ~p~n", [DR]),
	...
	_ ->
	    Stereo = false,
 	    {ok, D} = slave:start (net_adm:localhost(), 
				     'display',
				     " --env DISPLAY-:0 " ++
				     " -pa ebin/ -pa include/ -pa examples/ "),
	    ...
	    io:format("Screen: ~p~n",[Result]),
	    io:format (" ~p initialized~n", [D]),
    ...
    io:format("rosen main engine succesfully initialized~n"),
