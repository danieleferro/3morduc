...
\texttt{DistanceCalcInterface} and link it with the \texttt{DataManager}, who will automaticly
...
counterclockwise the "a" line in the robot center with a predefined angle (named "sweep angle")
...
%
The function to evaluate the score from the difference between the two angles is a Gaussian function center
...
In this way is always possible to discern the best one, by choosing the highest value returned.
%
...
indicate us where to center another Gaussian function, in order to obtain the score from the distance between
...
difference. The values obtained from the two Gaussian function, after adding them, form the global score.
%

%
To summarize the method aboveillustrated, we coupled every image with a score: the one with the maximum value
...
%
The images situated within the sweep area, but with an orientation angle that differs too much from the robot orientation angle, have to be discarded as the previous one: the score assigned will be -2 again.
...
choose as background when no other image for exogentric vision are available. The score coupled with the egocentric 
vision will be -1.
%
