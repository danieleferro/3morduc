...
%% other two concrete objects, which implement the \texttt{IDataLogic} and the \newline
%% \texttt{IImageSelector}.
%
...
%% \texttt{IDataLogic} allows to retrieve information about the image file selected for background
%% (see \texttt{image\_data} struct, again in figure \ref {fig:class_diagram}). We refer the reader to chapters
%% \ref{sub:howretrievedata} and \ref{sub:howbackgroundimage} for more details abouts \texttt{IDataLogic}'s
%% abstract method; chapters \ref{sub:datasource} and \ref{sub:metrics} for a concrete example.
%% %

%% %
%% The other reference, pointing to a \texttt{IImageSelector} implementation, is used to recall the algorithm
...
%% %
%% Since the unique method present in \texttt{IImageSelector} (named \texttt{Calculate}) is called by the
%% concrete instance of \texttt{IDataLogic}, \newline \texttt{DataManger} forwards its reference to the latter,
...
%% \texttt{IDataLogic} instance, in order to teleguide the robot and retrieve its new status
...
%% %
%% Later on, the concrete \texttt{IDataLogic} instance is asked again for the background image. After
