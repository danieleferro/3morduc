...
start_link (World = #world{}) ->
  ...
  rosen:add_world (World#world.rosen_name, WorldPid),
  {ok, WorldPid}.
...
init (World) ->
  Floor = floor (0.0, 0.0,
		 ...
		 World#world.height,
                 World#world.color,
		 World#world.rosen_name
		),
  {ok, WorldObjectPid} = object3d:new (#object3d { type = compound,
                                                   objects = [Floor],
						   rosen_name = World#world.rosen_name 
						  }),
...
handle_call ({paint_floor, X, Y, Width, Height, Color}, _, World) ->
  FloorObject = floor (X, Y, Width, Height, Color, World#world.rosen_name),
...
%%
%%
floor (X, Y, Width, Height, Color, Rosen_name) ->
  #object3d { type = box,
              width = Width,
              height = 0.0,
              depth = Height,
