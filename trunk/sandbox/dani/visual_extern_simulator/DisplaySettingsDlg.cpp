...
void CDisplaySettingsDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Check(pDX, IDC_ESTEREO, m_EnableStereo);
	//}}AFX_DATA_MAP
}
...
{	
	...
	{
		theApp.m_Width			= mode->dwWidth;//+8;				//pass vars to main app
		theApp.m_Height			= mode->dwHeight;//+34;
		theApp.m_bitsPerPixel	= mode->dwBpp ;

		theApp.m_isFullScreen   = m_bFullscreen; 
		theApp.m_EnableStereo =m_EnableStereo;
		CDialog::OnOK();
	}
...
void CDisplaySettingsDlg::EnumDisplays()
{
	...
	while( ::EnumDisplaySettings( NULL , dwMode , &dm ) )		//loop modes
	{
		...
		{
			if( dm.dmBitsPerPel == 16 ||							// 16 or 32 m ?
				dm.dmBitsPerPel == 32 )
			{
