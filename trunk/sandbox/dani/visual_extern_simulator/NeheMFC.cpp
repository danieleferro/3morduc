...
#define WINDOW_TITLE	"Virtual Robotics Rimulator"
/////////////////////////////////////////////////////////////////////////////
// CNeheMFCApp
			...
			{	// Initialize was a success
				...
				while (isMessagePumpActive == TRUE)						// While The Message Pump Is Active
				{
					...
					if (PeekMessage (&msg, m_wndMain.m_hWnd, 0, 0, PM_REMOVE) != 0)
					{
						...
						case WM_LBUTTONDOWN:
							{
								m_appMain.buttondown=1;
								break;
							}
						case WM_LBUTTONUP:
							{
								m_appMain.buttondown=0;
								m_appMain.dxold=m_appMain.dx;
								m_appMain.dyold=m_appMain.dy;
								break;
							}
						default:
							break;
					...
					else												// If There Are No Messages
					{
						...
						else											// If Window Is Visible
						{
							...
							m_appMain.HandleDisplay(toggle);


							//SwapBuffers (m_pDC->m_hDC);					// Swap Buffers (Double Buffering)

							m_appMain.HandleKeyboard(keyDown);


							/* code to save robot status */
							//m_appMain.SaveStatus();


							//FILIPPO					GetMessage(		&msg, m_wndMain.m_hWnd, WM_KEYFIRST , WM_KEYLAST );



							if (keyDown[VK_F1])								// Is F1 Being Pressed?
							{
								//keyDown[VK_F1]=FALSE;					// If So Make Key FALSE
								DestroyOpenGLWindow ();				// Destroy The Active Window								// Kill Our Current Window
								m_appMain.fullscreen=!m_appMain.fullscreen;		// Toggle Fullscreen / Windowed Mode
							...
							if (keyDown[VK_ESCAPE])						// Is ESC Being Pressed?
...
BOOL CNeheMFCApp::CreateOpenGLWindow()
{
	...
	PIXELFORMATDESCRIPTOR pfd =											// pfd Tells Windows How We Want Things To Be
	{
		...
		PFD_DOUBLEBUFFER |											// STEREO ADDED by FILIPPO
		(PFD_STEREO & m_EnableStereo << 1),												// Must Support Double Buffering
...
void CNeheMFCApp::ReshapeGL(int width, int height)						// Reshape The Window When It's Moved Or Resized
{
	...
	glLoadIdentity ();				
	m_appMain.camera.focallength=12.0f;// Reset The Projection Matrix
	gluPerspective (m_appMain.camera.focallength, (GLfloat)(width)/(GLfloat)(height),			// Calculate The Aspect Ratio Of The Window
	...
	theApp.m_Height=height;
	theApp.m_Width=width;
}
